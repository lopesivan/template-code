...
if [[ "$EXTENSION" == "" ]]; then
cat <<EOF
        self.tmpl.name = "%s/%s" % (self.tmpl.dir, self.tmpl.klass['name'])
EOF
else
cat <<EOF
        self.tmpl.name = "%s/%s.${EXTENSION}" % (self.tmpl.dir, self.tmpl.klass['name'])
EOF
fi
else
if [[ "$EXTENSION" == "" ]]; then
cat <<EOF
        self.tmpl.name = "%s/${DIR}/%s" % (self.tmpl.dir, self.tmpl.klass['name'])
EOF
else
cat <<EOF
        self.tmpl.name = "%s/${DIR}/%s.${EXTENSION}" % (self.tmpl.dir, self.tmpl.klass['name'])
EOF
fi
fi
