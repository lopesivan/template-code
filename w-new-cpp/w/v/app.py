...
def main():
    ...
    if os.path.isfile(path) and os.access(path, os.R_OK):
        ...
        t = models.TCMakeLists_txt(data_model, 'new_cpp.commmand.CMakeLists_txt')
        ...
        t = models.TDot_gitignore(data_model, 'new_cpp.commmand.dot_gitignore')
        if options.stdout == True :
            t.put()
        if options.save == True :
            t.save()
        ...
        t = models.TDot_local_vimrc(data_model, 'new_cpp.commmand.dot_local_vimrc')
        if options.stdout == True :
            t.put()
        if options.save == True :
            t.save()

        t = models.TDot_projections_json(data_model, 'new_cpp.commmand.dot_projections_json')
        if options.stdout == True :
            t.put()
        if options.save == True :
            t.save()
