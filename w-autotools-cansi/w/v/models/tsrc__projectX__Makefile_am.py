...
class TSrc__projectX__Makefile_am(Factory):
    """Classe de template."""
    def __init__(self, data_model, template_name):
        ...
        self.tmpl.name = "%s/%s" %(self.tmpl.autotools['dir'], "src/projectX/Makefile.am")
    ...
    def put(self):
        ...
        print self.tmpl
    def save(self):
        DIR = "%s/%s" %(self.tmpl.autotools['dir'], "src/projectX")
        sh = Sh(DIR)
        sh.mkdir()
        fileName = "%s" % self.tmpl.name
